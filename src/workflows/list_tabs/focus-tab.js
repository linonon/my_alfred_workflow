...
function run(args) {
  ...
  const { browser, browserWindow, maybeSystemWindow } = getBrowserAndWindows(windowIndex);
...
function getBrowserAndWindows(windowIndex) {
  ...
  const maybeSystemWindow = getSystemWindow(browserName, windowIndex, browserWindow.title());
...
function getSystemWindow(browserName, windowIndex, browserWindowTitle) {
  ...
  const systemWindowByIndex = getSystemWindowByIndex(browserProcess, windowIndex, expectedTitlePrefix);
 ...
 * undefined.
 */
function getSystemWindowByIndex(browserProcess, windowIndex, expectedTitlePrefix) {
...
function getSystemWindowByTitle(browserProcess, expectedTitlePrefix) {
  ...
  return systemWindows.find((systemWindow) => hasExpectedTitle(systemWindow, expectedTitlePrefix));
}
